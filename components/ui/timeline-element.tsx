...
const TimelineEvent: React.FC<TimelineEventProps> = ({ icon, title, description, date, isReversed, link, tags }) => {
  ...
  return (
    <motion.div
      ...
      className={`mb-4 flex flex-col-reverse lg:flex-row lg:justify-between lg:items-center w-full ${isReversed ? 'lg:flex-row-reverse' : ''}`}
    >
      <div className="order-1  w-full lg:w-5/12 md:order-0"></div>
      <div className="z-10 -ml-8 lg:-ml-0 flex items-center order-1 bg-shark-400 dark:bg-shark-800 shadow-xl w-10 h-10 rounded-full lg:order-1 lg:mb-0">
        <h1 className="mx-auto font-semibold text-lg text-gray-100 dark:text-white">
          {icon}
        </h1>
      </div>
      <div className="ml-12 lg:ml-0 order-1 h-auto bg-shark-50 dark:bg-shark-900 border border-shark-500 dark:border-shark-300 rounded-lg shadow-xl w-full lg:w-5/12 px-6 py-4 lg:order-2">
  ...
  return (
    <div className="max-w-full mx-auto">
      <div className="relative wrap overflow-hidden p-10 h-full">
        <div className="sm:absolute lg:flex hidden border-opacity-20 border-gray-800 dark:border-gray-300 h-full border" style={{ left: '50%' }}></div>
