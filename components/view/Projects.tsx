...
export default function ProjectsSection() {
  ...
  return (
    <section id="projekty"  className="scroll-mt-28 w-full mx-auto mt-48 mb-28">
      <div  >
        {/*<SectionTitle heading={texts['projectsTitle']} className="" subHeading={texts['projectsDescription']} />*/}

        <div ref={ref} className="text-center mt-20 mb-16">
          <p className="uppercase text-xs lg:text-md font-medium pb-4"> Skutečné použití </p>
          <h2 className="text-2xl lg:text-5xl  font-bold pb-2"> Projekty </h2>
          <p className="font-light text-black/70 dark:text-white/50"> Podívejte se na weby, které jsem vyvinul nebo se na nich z části podílel </p>
        </div>
        ...
        <div className="text-center mt-32">
          <p className="uppercase text-xs lg:text-md font-medium pb-4"> Pár příkladů </p>
          <h2 className="text-2xl lg:text-5xl font-bold pb-2"> Vedlejší projekty </h2>
          <p className="font-light text-black/70 dark:text-white/50"> Podívejte se na některé mé osobní projekty vytvořené ve volném čase </p>
        </div>
        ...
        <div className="mx-auto flex flex-col mt-8 gap-4">
          {sideProjectsData.map((project, index) => (
            <div 
              key={index} 
              className="w-full lg:w-2/3 mx-auto flex flex-col lg:flex-row justify-between bg-shark-800 rounded-xl shadow-md dark:border-white/40 min-h-72 overflow-hidden"
            >
              <div className="flex-1 p-8 w-full lg:w-1/2">
                ...
                <p className="border-b text-sm md:text-md pb-4 dark:border-white/40 opacity-70">
                  {project.description}
                </p>            
                ...
                <div className="grid grid-cols-2 text-xs md:grid-cols-2 lg:grid-cols-3  mt-4 gap-2 opacity-60">
                  {project.tags.map((tag, idx) => (
                    <span 
                      className="bg-shark-950 rounded-full py-1 text-center" 
                      key={idx}
                    >
                      {tag}
                    </span>
                ...
                <Link href={project.githubLink} target="_blank">
                  <Button 
                    size="lg" 
                    className="mt-8 px-8 dark:bg-black flex flex-row items-center gap-x-2 mr-4 group"
                  >
              ...
              <div 
                className="relative overflow-hidden w-full lg:w-1/2 mt-4 lg:mt-0"
              >          
                <Image 
