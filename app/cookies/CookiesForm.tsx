...
export default function CookiesForm() {
  ...
  return (
    <>
      <div className={`fixed bottom-2 overflow-hidden left-1/2 z-20 transform -translate-x-1/2 p-2 rounded-full w-[80%] bg-gray-200 dark:bg-gray-900 border border-gray-500 text-center flex flex-row items-center justify-between transition-transform duration-500 ${isClosed ? 'translate-y-full' : ''}`} style={{ right: isClosed ? '100%' : 'auto' }}>
        <p className='pl-2 text-sm flex flex-row items-center gap-x-4 text-gray-600 dark:text-gray-200'>
