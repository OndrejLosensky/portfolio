...
export default function CookiesForm() {
  ...
  return (
    <>
      <div
        className={`fixed bottom-2 overflow-hidden left-1/2 z-30 transform -translate-x-1/2 p-2 rounded-lg lg:rounded-full flex-col w-[90%] lg:w-[80%] bg-gray-200 dark:bg-gray-900 border border-gray-500 text-center flex lg:flex-row items-center justify-between transition-transform duration-500 ${
          isClosed ? "translate-y-full" : ""
        }`}
        style={{ right: isClosed ? "100%" : "auto" }}
      >
        ...
        <div className="flex flex-col lg:flex-row gap-x-2 items-center">
          <Link
            href="/"
            className="dark:text-bermuda-400 text-bermuda-600 mr-0 mt-2 lg:mt-0 lg:mr-2 text-xs underline underline-offset-2 font-semibold"
          >
            {texts["cookiesBtn1"]}
          </Link>
          <div className="my-2 lg:my-0 lg:mt-0 lg:space-x-2 mt-4 space-x-4">
            <button
              ...
              className="bg-bermuda-500 hover:bg-bermuda-600 transition duration-300 text-white rounded-full py-0.5 px-4"
            >
              {texts["cookiesBtn2"]}
            </button>
            <button
      ...
      {isClosed && (
        <div
          className="z-1 fixed hover:py-2 duration-300 bottom-0 left-2 py-0.5 px-2 bg-gradient-to-tr to-bermuda-600 from-emerald-400 rounded-t-full cursor-pointer flex items-center"
          onClick={handleReopen}
          title="Znovu otevřít nastavení cookies"
        >
